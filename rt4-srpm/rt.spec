...
# *STOP* trying to use a patch file to remove generated files, it's
# nasty and requires keeping the patch in complete sync with the tarball
#%patch1 -p1
%{__rm} -f Makefile
%{__rm} -f bin/rt
%{__rm} -f bin/rt-crontool
%{__rm} -f bin/rt-mailgate
%{__rm} -f etc/RT_Config.pm
%{__rm} -f etc/upgrade/3.8-ical-extension
%{__rm} -f etc/upgrade/4.0-customfield-checkbox-extension
%{__rm} -f etc/upgrade/generate-rtaddressregexp
%{__rm} -f etc/upgrade/split-out-cf-categories
%{__rm} -f etc/upgrade/switch-templates-to
%{__rm} -f etc/upgrade/upgrade-articles
%{__rm} -f etc/upgrade/vulnerable-passwords
%{__rm} -f lib/RT/Generated.pm
%{__rm} -f sbin/rt-attributes-viewer
%{__rm} -f sbin/rt-clean-sessions
%{__rm} -f sbin/rt-dump-metadata
%{__rm} -f sbin/rt-email-dashboards
%{__rm} -f sbin/rt-email-digest
%{__rm} -f sbin/rt-email-group-admin
%{__rm} -f sbin/rt-fulltext-indexer
%{__rm} -f sbin/rt-importer
%{__rm} -f sbin/rt-preferences-viewer
%{__rm} -f sbin/rt-serializer
%{__rm} -f sbin/rt-server
%{__rm} -f sbin/rt-server.fcgi
%{__rm} -f sbin/rt-session-viewer
%{__rm} -f sbin/rt-setup-database
%{__rm} -f sbin/rt-setup-fulltext-index
%{__rm} -f sbin/rt-shredder
%{__rm} -f sbin/rt-test-dependencies
%{__rm} -f sbin/rt-validate-aliases
%{__rm} -f sbin/rt-validator
%{__rm} -f sbin/standalone_httpd
%{__rm} -f t/data/configs/apache2.2+fastcgi.conf
%{__rm} -f t/data/configs/apache2.2+mod_perl.conf

%patch2 -p1
# No longer needed with 4.2.11
#%patch3 -p1
# fix '/usr/bin/env perl' usage automatically
grep -rl '#!/usr/bin/env perl' . | while read name; do
     echo "Fixing '#!/usr/bin/env perl' in $name"
...
# Silence rpmlint
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/3.8-ical-extension
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/4.0-customfield-checkbox-extension
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/generate-rtaddressregexp
#chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/shrink_cgm_table.pl
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/shrink-cgm-table
#chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/shrink_transactions_table.pl
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/split-out-cf-categories
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/switch-templates-to
#chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/time-worked-history.pl
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/upgrade-mysql-schema.pl
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/vulnerable-passwords
chmod a+x ${RPM_BUILD_ROOT}%{_datadir}/%{name}/upgrade/upgrade-articles
